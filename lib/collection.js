...
let errors = {
  ...
  blockDataWrong: 'Block data wrong',
  blockUndefined: 'Block undefined'
}
...
Collection.prototype.update = function (id, stats, callback) {
  ...
  if (!node) {
    callback(errors.nodeNotFound, null)
  } else {
    var block = this._blockchain.add(stats.block, id, node.trusted)

    if (!block) {
      callback(errors.blockDataWrong, null)
    } else {
...
Collection.prototype.addBlock = function (id, stats, callback) {
  ...
  if (!node) {
    ...
    if (!block) {
      callback(errors.blockUndefined, null)
    } else {
      var propagationHistory = this._blockchain.getNodePropagation(id)
...
Collection.prototype.removeOldNodes = function () {
  ...
  if (deleteList.length > 0) {
    for (var j = 0; i < deleteList.length; i++) {
      this._items.splice(deleteList[j], 1)
    }
  }
}
